...
class egreso extends CI_Controller
{
    ...
    if (!$this->acciones(10)){redirect(base_url()."inicio");}
	...
	{
    $controlip=$this->controlip('egreso');
    ...
    $empresa=$this->empresa_model->mostrar();
		...
		$listas=$this->egreso_model->mostrarTotal($filtros);
		$this->layout->setTitle("Egreso");
		$this->layout->view("index",compact("anexos","nestablecimiento",'empresa',"listas","inicio","fin"));

	}

	public function egresoi($id=null)
	{
    $controlip=$this->controlip('egreso/egresoi');
		if ($this->input->post())
		{
			$data=array
			(
			...
			$insertar=$this->egreso_model->insert($data);
			$this->session->set_flashdata("css", "success");
			$this->session->set_flashdata("mensaje", "Los datos se han guardado exitosamente!");
      $control_movimiento=$this->movimientos('egreso/egresoi','Registro egreso nro '.$insertar);
			echo base_url()."egreso";
		}

		$comprobantes=$this->tegreso_model->mostrarTotal();
		$mpagos=$this->tpago_model->mostrarTotal();
		$this->layout->setLayout("blanco");
		$this->layout->view("egresoi",compact("mpagos","comprobantes"));
	}

	public function egresod($id)
	{
		if (!$id) {show_404();}
    $controlip=$this->controlip('egreso/egresod');
		$datos=$this->egreso_model->mostrar($id);
		if ($datos==NULL) {show_404();}

		$eliminar=$this->egreso_model->delete($id);
    $control_movimiento=$this->movimientos('egreso/egresod','Elimino egreso nro '.$id);
    $success=true;
    $titulo='Borrado!';
    ...
    $proceso['color']=$color;
    $proceso['url']=base_url().'egreso';
    echo json_encode($proceso);
    exit();
	}

	public function pdfegreso($inicio,$fin)
	{
    $nestablecimiento=$this->establecimiento_model->mostrar($this->session->userdata("predeterminado"));
		...
		$listas=$this->egreso_model->mostrarTotal($filtros);
		$this->layout->setLayout("blanco");
		$this->layout->view("pdfegreso",compact("empresa","nestablecimiento","listas","inicio","fin"));
	}

	public function excelegreso($inicio,$fin)
	{
		$filtros=array("idestablecimiento"=>$this->session->userdata("predeterminado"),"femision>="=>$inicio,"femision<="=>$fin);
		if ($this->session->userdata("tipo")!='admin') {$filtros['iduser']=$this->session->userdata("id");}
		$listas=$this->egreso_model->mostrarTotal($filtros);
    ...
    header('Content-Disposition: attachment;filename="LISTA_EGRESOS.xlsx"');
