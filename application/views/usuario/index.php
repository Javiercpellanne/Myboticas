<div class="content-header pb-1">
  <div class="container-fluid">
    <div class="row mb-0">
      <div class="col-sm-6">
        <h4 class="m-0 text-dark">Usuario <button type="button" class="btn btn-info btn-sm py-0" onclick="mostrarModal('<?php echo base_url(); ?>usuario/usuariosi','bdatos')"><i class="fa fa-plus"></i> Nuevo Usuario</button></h4>
      </div>
      <div class="col-sm-6">
        ...
        <div class="card card-outline card-primary">
          <div class="card-header py-2">
            <ul class="nav nav-pills">
              <li class="nav-item"><a class="nav-link py-1 active" href="<?php echo base_url(); ?>usuario">Activos</a></li>
            ...
            <table id="sampleTable" class="table table-striped table-bordered table-sm dt-responsive nowrap" style="width:100%">
              ...
              <tbody>
                <?php foreach ($listas as $lista) { ?>
                  <?php
                  $idestablecimiento=explode(',',$lista->idestablecimiento);
                  $establecimientos=array();
                  for ($i=0; $i < count($idestablecimiento) ; $i++) {
                    $nestablecimiento=$this->establecimiento_model->mostrar($idestablecimiento[$i]);
                    array_push($establecimientos,$nestablecimiento->descripcion);
                  }
                  $establecimientos=implode(', ',$establecimientos);
                  ?>
                  <?php if ($lista->id>1): ?>
                  <tr>
                    <td><?php echo $lista->id; ?></td>
                    <td><?php echo $lista->nombres; ?></td>
                    <td><?php echo $establecimientos; ?></td>
                    ...
                    <td>
                      <div class="btn-group">
                        ...
                        <?php if ($lista->id>2): ?>
                          <button type="button" class="btn btn-success btn-sm py-0" onclick="mostrarModal('<?php echo base_url(); ?>usuario/acceso/<?php echo $lista->id; ?>','bdatos','Acceso Modulo')" title="Acceso Modulo" data-toggle="tooltip" data-placement="bottom"><i class="fa fa-sign-out-alt"></i></button>

                          <a href="<?php echo base_url(); ?>usuario/deshabilitar/<?php echo $lista->id; ?>" class="btn btn-outline-danger btn-sm py-0" data-toggle="tooltip" data-placement="bottom" title="Inactivar"><i class="fa fa-thumbs-down"></i></a>
                        <?php endif ?>
                      </div>
                    </td>
                  </tr>
