<?php
$altura=$detalles!=null ? 180+(count($detalles)*5) : 180;
...
$tblf = '<table cellspacing="0" cellpadding="2" border="0">
    <tr>
        <td width="40%"><b>F. Emision :</b></td>
        <td width="60%">'.$datos->femision.' '.$datos->hemision.'</td>
    </tr>
    <tr>
        <td><b>Cliente :</b></td>
...
$pdf->write2DBarcode($qrcode, 'QRCODE,Q', '','', 20, 20, $style, 'N');

$pdf->SetY($inicio+2);
if ($datos->hash!='') {
    $pdf->SetFont('dejavusanscondensed','B',6);
    $pdf->Cell(20,2,'',0,0,'L');
    $pdf->Cell(15,2,'Codigo Hash : ',0,1,'L');

    $pdf->SetFont('dejavusanscondensed','',6);
    $pdf->Cell(20,2,'',0,0,'L');
    $pdf->Cell(0,2,$datos->hash,0,1,'L');
    $pdf->Ln(1);
}
...
$pdf->SetFont('dejavusanscondensed','B',6);
$pdf->Cell(20,2,'',0,0,'L');
$pdf->Cell(0,2,'Condicion Pago: '.$tpago,0,1,'L');

if ($datos->condicion==1) {
    $pdf->SetFont('dejavusanscondensed','B',6);
    $pdf->Cell(20,2,'',0,0,'L');
    $pdf->Cell(0,2,'Pagos : ',0,1,'L');

    $pdf->SetFont('dejavusanscondensed','',6);
    foreach ($cobros as $cobro) {
        $pdf->Cell(20,2,'',0,0,'L');
        $pdf->Cell(0,2,'• '.$cobro->ntpago.' -- S/ '.$cobro->total,0,1,'L');
    }
} else {
    $pdf->SetFont('dejavusanscondensed','B',6);
    $pdf->Cell(20,2,'',0,0,'L');
    $pdf->Cell(0,2,'Cuota / Fecha / Monto',0,1,'L');
    $pdf->SetFont('dejavusanscondensed','',6);
    ...
    for ($i=1; $i <= $datos->cuotas ; $i++) {
      ...
      $pdf->Cell(20,2,'',0,0,'L');
      $pdf->Cell(0,2,'• # '.$i.' / '.$feinicial.' / S/ '.$cuota,0,1,'L');
...
$pdf->SetFont('dejavusanscondensed','B',6);
$pdf->Cell(20,2,'',0,0,'L');
$pdf->Cell(15,2,'Vendedor : ',0,0,'L');
$pdf->SetFont('dejavusanscondensed','',6);
$pdf->Cell(0,2,($vendedor->nombres??''),0,1,'L');
...
if ($tpuntos->cantidad!=null) {
    $pdf->SetFont('dejavusanscondensed','B',6);
    $pdf->Cell(30,2,'Puntos Acumulados : ',0,0,'L');
    $pdf->SetFont('dejavusanscondensed','',6);
    $pdf->Cell(0,2,$tpuntos->cantidad,0,1,'L');
}
