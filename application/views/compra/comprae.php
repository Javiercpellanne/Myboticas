      ...
      <div class="col-12">
        <div class="card card-outline card-primary">
          <div class="card-body p-3">
            ...
            <?php echo form_open(null,array("class"=>"form-horizontal","name"=>"form1", "id"=>"form1", "autocomplete"=>"off", "onsubmit"=>"return envioFormulario('".base_url()."compra/actualizar/".$id."');")); ?>
              <div class="form-group row mb-1">
                ...
                <div class="col-sm-2">
                  <select name="comprobante" id="comprobante" class="form-control form-control-sm" required>
                    <option value="" <?php echo set_value_select($datos,'comprobante','',$datos->comprobante) ?>>::Selec</option>
                    <option value="01" <?php echo set_value_select($datos,'comprobante','01',$datos->comprobante) ?>>Factura</option>
                    <option value="03" <?php echo set_value_select($datos,'comprobante','03',$datos->comprobante) ?>>Boleta</option>
                    <option value="09" <?php echo set_value_select($datos,'comprobante','09',$datos->comprobante) ?>>Guia Remision</option>
                    <option value="99" <?php echo set_value_select($datos,'comprobante','99',$datos->comprobante) ?>>Nota Venta</option>
                  </select>
                </div>
              ...
              <div class="form-group row mb-1">
                ...
                <input name="idproveedor" id="idproveedor" type="hidden" value="<?php echo $datos->idproveedor ?>"/>
                <div class="col-sm-4">
                  <div class="input-group">
                ...
                <div class="col-sm-2">
                  <div class="custom-control custom-switch mt-1">
                      <input class="custom-control-input" name="incluye" type="checkbox" id="incluye" value="1" onclick="calcularCompra();" <?php echo set_value_check($datos,'incluye',$datos->incluye,1); ?>>
                ...
                <div class="col-sm-2">
                  <div class="btn-group">
                    <button id="buscar" type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#buscompra"><i class="fa fa-cart-plus"></i> <small>AGREGAR PROD</small></button>

                    <button type="button" class="btn btn-primary btn-sm" onclick="mostrarModal('<?php echo base_url(); ?>nventa/productoi','bdatos','Datos del Producto')" title="Producto Nuevo" data-toggle="tooltip" data-placement="bottom"><i class="fas fa-plus"></i> <small>NUEVO PROD</small></button>
                  </div>
                </div>
              </div>
              ...
              <div class="table-responsive p-0" style="height: 370px;">
                <table class="table table-hover table-sm">
                  ...
                  <tbody id="grilla">
                    ...
                    $color= $detalle->tafectacion==15 ? ' class="table-primary"' : '';
                    ?>
                    <tr id="item<?php echo $i; ?>" <?php echo $color; ?>>
                      <td>
                      ...
                      <td><input type="text" name="lote[]" value="<?php echo $detalle->lote; ?>" class="<?php echo $estilo ?>"/></td>
                      <td><input type="<?php echo $tinput ?>" name="fvencimiento[]" value="<?php echo $detalle->fvencimiento; ?>" class="<?php echo $estilo ?>"/></td>
                      <td>
                        <input type="hidden" class="factores" name="factor[]" id="factor[]" value="<?php echo $detalle->factor ?>" class="factores">
                        <input type="hidden" name="almacenc[]" value="<?php echo $detalle->calmacen; ?>" class="calmacenes"/>
                        <input type="hidden" name="almacenp[]" value="<?php echo $detalle->palmacen; ?>" class="palmacenes"/>
                      ...
                      <td><input type="text" name="cantidad[]" value="<?php echo $detalle->cantidad ?>" min="1" class="form-control form-control-sm cantidades"/></td>
                      <td>
                        <input type="text" name="precio[]" value="<?php echo $detalle->precio ?>" class="form-control form-control-sm precios"/>
                        <input type="hidden" name="pventa[]" value="">
                        <input type="hidden" name="venta[]" value="">
                        <input type="hidden" name="blister[]" value="">
                      </td>
                      <td><input type="text"  name="importe[]" value="<?php echo $detalle->importe ?>" class="campo text-right importes"/></td>
                      <td><a href="javascript:void(0)" class="btn btn-danger btn-sm py-0" onclick="borrarCompra('<?php echo base_url(); ?>compra/eliminar/<?php echo $detalle->id; ?>','item<?php echo $i; ?>')" title="Eliminar"><i class="fa fa-trash"></i></a></td>
                    </tr>
                    <?php $i++; ?>
                    <?php endforeach ?>
                  </tbody>
              ...
              <table cellpadding="3">
                <tr>
                  ...
                  <td width="10%">
                    ...
                    <h4><input name="subtotal" type="text" id="subtotal" class="campo text-right" value="<?php echo $datos->subtotal; ?>"/></h4>
                  </td>
                  <td width="10%" align="right"><strong>IGV </strong></td>
                  <td width="10%">
                    <h4><input name="igv" type="text" id="igv" class="campo text-right" value="<?php echo $datos->igv; ?>"/></h4>
                  </td>
                  <td width="10%" align="right"><strong>TOTAL </strong></td>
                  <td width="10%">
                    <h4><input name="total" type="text" id="total" class="campo text-right" value="<?php echo $datos->total; ?>"/></h4>
                  </td>
                  <td width="10%" align="right"><b>PERCEPCION </b></td>
                  <td width="10%">
                    <input name="mpercepcion" type="text" id="mpercepcion" class="form-control form-control-sm text-right"onkeyup="suma('total','mpercepcion','pagar');" value="<?php echo $datos->percepcion>0 ? $datos->percepcion: ''; ?>"/>
                  </td>
                  <td width="10%" align="right"><strong>PAGAR </strong></td>
                  <td width="10%">
                    <h4><input name="pagar" type="text" id="pagar" class="campo text-right" value="<?php echo $datos->percepcion>0 ? $datos->percepcion+$datos->total: ''; ?>" readonly/></h4>
                  </td>
                </tr>
              </table>
...
<div class="modal fade" id="buscompra">
