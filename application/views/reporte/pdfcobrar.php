...
$pdf->SetTitle('Reporte Deudas Clientes');
...
$tblc = '<table cellspacing="0" cellpadding="0" border="0">
    <tr>
        ...
        <td width="70%" align="center" height="48px"><br> DEUDA DE : '.$this->input->post('cliente',true).'<br>DESDE '.FormatoFecha($this->input->post('cinicio',true)).' AL '.FormatoFecha($this->input->post('cfin',true)).'</td>
...
$tbl = '<table cellspacing="0" cellpadding="1" border="0">
        <tr>
            ...
            <th align="center" width="15%" style="border: 1px solid #000;"><strong>Cobrado</strong></th>
            <th align="center" width="15%" style="border: 1px solid #000;"><strong>Saldo</strong></th>
        </tr>';
    $pdf->SetFont('helvetica','',8);
    $i=1;
    $mtotal=0; $mcobrado=0; $msaldo=0;
    foreach ($datos as $dato) {
        $cobros=$this->cobro_model->mostrarTotal(array("idnventa"=>$dato->id));
        $cobrado=$this->cobro_model->montoTotal(array("idnventa"=>$dato->id));
        $saldo=$dato->total-($cobrado->total??0);
        $tbl .= '<tr>
            ...
            foreach ($cobros as $cobro) {
                $tbl .= '<tr>
                    <td align="right">'.$cobro->femision.'</td>
                    <td> S/. '.$cobro->total.'</td>
                </tr>';
            }
            $tbl .= '</table></td>
            <td align="right" style="border-top: 1px solid #000;">S/. '.$dato->total.'</td>
            <td align="right" style="border-top: 1px solid #000;">'.($cobrado->total??0).'</td>
            <td align="right" style="border-right: 1px solid #000; border-top: 1px solid #000;">'.formatoPrecio($saldo).'</td>
        </tr>';
        $mtotal+=$dato->total; $mcobrado+=($cobrado->total??0); $msaldo+=$saldo;
        $i++;
    }

    foreach ($comprobantes as $dato) {
        $cobros=$this->cobroe_model->mostrarTotal(array("idventa"=>$dato->id));
        $cobrado=$this->cobroe_model->montoTotal(array("idventa"=>$dato->id));
        $saldo=$dato->total-($cobrado->total??0);
        $tbl .= '<tr>
            <td style="border-left: 1px solid #000; border-top: 1px solid #000;">'.$dato->femision.'</td>
            <td style="border-top: 1px solid #000;">'.$dato->serie.'-'.$dato->numero.'</td>
    ...
    $tbl .= '<tr>
        ...
        <td align="right" style="border-top: 1px solid #000;">'.formatoPrecio($mcobrado).'</td>
